...
// the optional characteristics are: NAME, STATUS_ACTIVE, STATUS_FAULT,
//     STATUS_TAMPERED, STATUS_LOW_BATTERY
// See HAP section 8.41 and characteristics.h

// (required) format: float; HAP section 9.35;
// min 0, max 100, step 0.1, unit celsius
homekit_characteristic_t	g_cha_current_temperature
	 = HOMEKIT_CHARACTERISTIC_(CURRENT_TEMPERATURE, 0);

// (optional) format: string; HAP section 9.62; max length 64
homekit_characteristic_t	g_cha_name
	 = HOMEKIT_CHARACTERISTIC_(NAME, "Temperature Sensor");

// (optional) format: bool; HAP section 9.96
// homekit_characteristic_t cha_status_active
// 	 = HOMEKIT_CHARACTERISTIC_(STATUS_ACTIVE, true);

// (optional) format: uint8; HAP section 9.97; 0 "No Fault", 1 "General Fault"
// homekit_characteristic_t cha_status_fault
// 	 = HOMEKIT_CHARACTERISTIC_(STATUS_FAULT, 0);

// (optional) format: uint8; HAP section 9.100;
// 0 "Accessory is not tampered", 1 "Accessory is tampered with"
// homekit_characteristic_t cha_status_tampered
// 	 = HOMEKIT_CHARACTERISTIC_(STATUS_TAMPERED, 0);

// (optional) format: uint8; HAP section 9.99;
// 0 "Battery level is normal", 1 "Battery level is low"
// homekit_characteristic_t cha_status_low_battery
